# Glucose‑Prediction‑NeuralODE

> 
> Personalized blood glucose prediction model based on Neural‑ODE with physiological prior constraints
> 基于 Neural‑ODE 并引入生理先验约束的个性化血糖预测模型

## 1. Project Background

### 问题背景

传统血糖仿真模型（例如双指数 IOB/COB 模型）采用**全局固定药代动力学参数**，所有患者共享同一套吸收、衰减常数。
**存在缺陷：**

1. 个体差异性被忽略：不同患者胰岛素吸收速率、碳水代谢敏感度、昼夜节律、运动响应、体内葡萄糖稳态自调节能力各不相同；全局一刀切参数容易造成极端血糖场景预测失真，高低血糖误差偏大。
2. 现实扰动因素考虑不全：原有模型较少显式建模昼夜环境、睡眠、黎明现象、运动带来的血糖扰动；运动诱发低血糖、餐后峰值偏移等场景误差明显。
3. 纯白箱生理模型表达能力有限，完全数据驱动神经网络又缺少医学约束，容易产出违背生理常识的预测轨迹。
